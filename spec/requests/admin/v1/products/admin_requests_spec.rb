...
RSpec.describe "Admin V1 Products as :admin", type: :request do
  ...
  context "POST /products" do
    let(:url) { "/admin/v1/products" }
    let(:categories) { create_list(:category, 2) }
    let(:system_requiriment) { create(:system_requiriment) }
    let(:post_header) { auth_header(user, merge_with: { 'Content-Type' => 'multipart/form-data' }) }
    
    context "with valid params" do
      let(:game_params) { attributes_for(:game, system_requiriment_id: system_requiriment.id) }
    ...
    context "with invalid :productable params" do
      let(:game_params) { attributes_for(:game, developer: "", system_requiriment_id: system_requiriment.id) }
